...
class TestWorkResidentPermitRenewalHistoryService(TestCase):
    ...
    def test_work_resident_permit_renewal_when_should_create_record(self):
        """
        Check if work permit renewal can created after submitting the initial application.
        """
        ...
        app_renewal_dto = RenewalApplicationDTO(
            applicant_identifier=self.application.application_document.applicant.user_identifier,
            document_number=self.document_number,
            process_name=ApplicationProcesses.WORK_RESIDENT_PERMIT.value
        )
        app_renewal_dto.work_place = "01"

        renewal_service = RenewalApplicationService(renewal_application=app_renewal_dto)
        renewal_service.process_renewal()

        counts = ApplicationRenewal.objects.filter(
            previous_application__application_document__document_number=self.document_number).count()
        self.assertGreater(counts, 0)

        application_renewal_history_count = ApplicationRenewalHistory.objects.all().count()
        self.assertGreater(application_renewal_history_count, 0)
        self.assertGreater(len(ApplicationRenewalHistory.objects.first().historical_record['data']), 0)
        ...
        application_renewal_history = ApplicationRenewalHistory.objects.get(
            ...
            application_user=self.application.application_document.applicant,
            process_name=ApplicationProcesses.WORK_RESIDENT_PERMIT.value
        )
        self.assertIsNotNone(application_renewal_history)
