...
class Command(CustomBaseCommand):
    ...
    process_name = ApplicationProcesses.VARIATION_PERMIT.value
        ...
        with atomic():
            ...
            for _ in range(50):
                ...
                VariationPermit.objects.get_or_create(
                    ...
                    draw_salary=faker.random_element(
                        elements=("yes", "no")
                    ),
                    salary_per_annum=faker.pydecimal(left_digits=5, right_digits=2, positive=True),
                    new_company_employee_count=faker.random_int(min=1, max=100),
                    new_company_registration=faker.date_between(start_date="-10y", end_date="today"),
                    man_power_projection=faker.text(),
                    amount_invested=faker.pydecimal(left_digits=7, right_digits=2, positive=True),
