...
class CreateTaskService:
    ...
    def read_task_details(self):
        ...
        try:
            ...
            config_location = os.path.join(os.getcwd(), "app_checklist", "data", "tasks", file_name)

            reader = ReadJSON(file_location=self.request_dto.task_details_config_file or config_location)
            self.logger.info("Successfully read rules from JSON file: %s", self.request_dto.task_details_config_file)
    ...
    def system_parameter(self):
        try:
            return SystemParameter.objects.get(application_type="DEFERRED_APPLICATION_DURATION")
