...
class AssessmentCaseNoteViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        self.logger.info(
            f"Updating case note {instance.parent_object_id} with data: {request.data}"
        )
        try:
            with transaction.atomic():
                ...
                self.logger.info(
                    f"Case note {instance.parent_object_id} updated successfully"
                )
            ...
            self.logger.error(
                f"Case note with ID {instance.parent_object_id} does not exist."
            )
