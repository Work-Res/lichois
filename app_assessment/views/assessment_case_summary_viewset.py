    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if not validator.is_valid():
                logger.warning("Validation failed for create request: %s", validator.response.result())
            ...
            if summary_service.response.status:
                ...
                logger.warning("Failed to create assessment case summary: %s", summary_service.response.result())
                return Response(summary_service.response.result(), status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error("Exception occurred while creating assessment case summary: %s", e, exc_info=True)
    ...
    def save(self, request):
        try:
            ...
            if not validator.is_valid_to_update():
                logger.warning("Validation failed for save request: %s", validator.response.result())
                return Response(validator.response.result(), status=status.HTTP_400_BAD_REQUEST)
            ...
            if summary_service.response.status:
                ...
                return Response(summary_service.response.result(), status=status.HTTP_200_OK)
            else:
                logger.warning("Failed to update assessment case summary: %s", summary_service.response.result())
                return Response(summary_service.response.result(), status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error("Exception occurred while updating assessment case summary: %s", e, exc_info=True)
