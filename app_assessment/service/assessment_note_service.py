...
class AssessmentNoteService:
    ...
    def create(self):
        self.logger.info(f"Creating a case note for {self.note_request_dto.document_number}")
        try:
    ...
    def update(self):
        ...
        try:
            with transaction.atomic():
                ...
                self.logger.debug(f"Fetched case note for {self.note_request_dto.document_number}")

                updated_data = self.note_request_dto.__dict__
                for key, value in updated_data.items():
                    setattr(case_note, key, value)
                case_note.save()
                self.logger.info(f"Updated case note for {self.note_request_dto.document_number}")
                api_message = APIMessage(
                    code=200,
            ...
            self.logger.error(f"Case note with document number  {self.note_request_dto.document_number} does not exist.")
        except IntegrityError as ex:
            self.logger.error(f"Transaction failed and was rolled back. {ex}")
