...
class BatchModelViewSetTest(BaseSetup):
    ...
    def create_board(self, region):
        ...
        board = Board.objects.create(
            name="Test Board", region=region, description="A test board"
        )
    ...
    def create_batch(self, meeting):
        ...
        data = {"meeting": meeting.id, "name": "New Test Batch"}
        self.client.post(url, data, format="json")
    ...
    def test_remove_application(self):
        ...
        data = {"application_id": version.application.id}
        response = self.client.post(url, data, format="json")
    ...
    def test_declare_conflict_of_interest(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            response.data["detail"], "Conflict of interest declared successfully"
        )
    ...
    def test_declare_no_conflict_for_all(self):
        ...
        print(url)
        attendee = Attendee.objects.get(meeting=self.meeting, member=self.member)
        data = {"meeting_session_id": meeting_session.id}
        print("for url", url)
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        count = ConflictOfInterest.objects.filter(
            attendee=attendee, has_conflict=False
        ).count()
    ...
    def test_conflict_of_interest_when_completed(self):
        ...
        url = reverse("citizenship:batch-declare-no-conflict-for-all", args=[batch.id])
        attendee = Attendee.objects.get(meeting=self.meeting, member=self.member)
        data = {"attendee_id": attendee.id, "meeting_session_id": meeting_session.id}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
