...
class MeetingService:
    ...
    @transaction.atomic
    def create_meeting(
        title, board, location, agenda, start_date, end_date, time, description=None
    ):
        try:

            if MeetingService.check_meeting_conflicts(board, start_date, end_date):
                raise ValidationError(
                    "A meeting for the same board already exists in the specified period."
                )
    ...
    def add_attendee(meeting, member_id, confirmed=False):
        try:
            ...
            attendee, created = Attendee.objects.get_or_create(
                member=member, meeting=meeting, defaults={"confirmed": confirmed}
            )
            if created:
    ...
    def remove_attendee(meeting_id, member_id):
        try:
            ...
            logger.error(
                f"Attendee does not exist for session {meeting_id} and member {member_id}"
            )
    ...
    def change_meeting_status(meeting_id, new_status):
        try:
            ...
            meeting.save()
            logger.info(
                f"Meeting status changed from {old_status} to {new_status} for meeting {meeting_id}"
            )
            return meeting
    ...
    def confirm_attendance(meeting_id, member_id, confirmed=False, proposed_date=None):
        try:
            ...
            if proposed_date:
                ...
                logger.info(
                    f"Attendee {attendee} proposed a new date for meeting {meeting_id}: {proposed_date}"
                )
            else:
                attendee.confirmed = confirmed
                attendee.proposed_date = None
                logger.info(
                    f"Attendee {attendee} confirmed attendance for meeting {meeting_id}"
                )
            attendee.save()
            return attendee
        except Attendee.DoesNotExist:
            logger.error(
                f"Attendee does not exist for meeting {meeting_id} and member {member_id}"
            )
            raise ValidationError("Attendee does not exist.")
        except Exception as e:
    ...
    def create_attendances_for_meeting(meeting):
        try:
            ...
            for member in members:
                ...
                logger.info(
                    f"Attendee created: {member.user.username} for meeting {meeting.title}"
                )
        except Exception as e:
    ...
    def create_session(meeting_id, title, date, start_time, end_time):
        try:
            meeting = Meeting.objects.get(id=meeting_id)
            if MeetingService.check_session_conflicts(
                meeting, date, start_time, end_time
            ):
                raise ValidationError(
                    "Meeting Session times conflict with an existing session."
                )
