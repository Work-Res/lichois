...
class IntentionFSLetterProductionProcess(ProductionProcess):
    def __init__(self, handler: UploadDocumentProductionHandler, context_generator: IntentionFSContextGenerator):
    ...
    def handle(self, application, decision):
        ...
        template_path = os.path.join(
            "citizenship", "data", "production", "templates", f"intention_fs_{status}_template.docx")
        document_output_path = os.path.join(
            settings.MEDIA_ROOT, f'intention_fs_{application.application_document.document_number}_{date_string}_{status}.docx')
