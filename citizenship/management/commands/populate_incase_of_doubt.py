...
class Command(CustomBaseCommand):
    ...
    process_name = CitizenshipProcessEnum.DOUBT_CITIZENSHIP_CERTIFICATE.value
    application_type = CitizenshipProcessEnum.DOUBT_CITIZENSHIP_CERTIFICATE.value

    def create_attachments(self):
        attachments = ChecklistClassifier.objects.get(
            code="CERTIFICATE_OF_CITIZENSHIP_INCASE_DOUBT_ATTACHMENT_DOCUMENTS"
        )
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(50):
            ...
            with atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Successfully populated Certificate in case of doubt data"
                    )
                )
