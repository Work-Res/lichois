...
class Form4(ApplicationBaseModel, DeclarationModelMixin, CommissionerOathModelMixin):
    ...
    primary_school_attend = models.CharField(
        max_length=200,
        null=True,
        blank=True,
    ...
    is_witness_signed = models.BooleanField(verbose_name="Witness Signature")

    witness_date_of_signature = models.DateField(
        null=True, blank=True, verbose_name="Date of witness's Signature"
    )

    countries_visited = models.ManyToManyField(
        CountryVisit, related_name="countries_visited"
    )
