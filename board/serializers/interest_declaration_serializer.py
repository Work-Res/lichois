...
from board.serializers.meeting_attendee_serializer import MeetingAttendeeSerializer
...
class InterestDeclarationSerializer(serializers.ModelSerializer):
    ...
    meeting_attendee = MeetingAttendeeSerializer()
    ...
    def to_internal_value(self, data):
        ...
        try:
            meeting_attendee = MeetingAttendee.objects.get(
                Q(board_member=board_member)
                & Q(attendance_status=PRESENT)
                & Q(meeting=mutable_data.get("meeting"))
            )
            ...
            try:
                InterestDeclaration.objects.get(
                    Q(meeting_attendee=meeting_attendee)
                    & Q(document_number=data.get("document_number"))
                )
