...
class BoardMeetingVoteViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="participants/(?P<document_number>[A-Za-z0-9-]+)",
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="tie-breaker/(?P<document_number>[A-Za-z0-9-]+)",
        url_name="tie-breaker",
    )
    def create_tie_breaker(self, request, document_number):
        meeting_vote_manager = self.get_meeting_vote_manager(request, document_number)
        tie_breaker = request.data.get("tie_breaker")
        print("********** tie- breaker from request **********", tie_breaker)
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="my-vote/(?P<document_number>[A-Za-z0-9-]+)",
        url_name="my-vote",
    )
    def my_vote(self, request, document_number):
        ...
        return Response(
            APIMessage(message="Vote not found", code=404).to_dict(), status=404
        )

    def create(self, request, *args, **kwargs):
        # if vote exists, raise an error
        meeting_vote_manager = self.get_meeting_vote_manager(
            request, request.data.get("document_number")
        )
        if meeting_vote_manager.vote_exists():
            return Response(
                APIMessage(message="Vote already exists", code=500).to_dict(),
                status=500,
            )
