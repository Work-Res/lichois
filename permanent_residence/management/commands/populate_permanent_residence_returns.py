...
class Command(CustomBaseCommand):
    help = (
        "Populate data for Populate data for Permanent residence for 10 years service"
    )
    process_name = ApplicationProcesses.PERMANENT_RESIDENCE_RETURNS.value
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(150):
            ...
            with atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Successfully populated permanent residence returns data"
                    )
                )
